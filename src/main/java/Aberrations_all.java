...
//  Panel panel;
 ...
 public static int INTERPOLATE_SUBDIV=   1; // interpolate kernels between defined by FFTOverlap
 ...
 public static double simul_fill=0.5; // part of the (center) pixel area being "photosensitive"
 ...
 public static double minCorrContrast=5.0; // Discrimination threshold between good and bad pattern correlation
 ...
 public static int    multiplesToTry=4; // try this number of maximums proportionally farther from 0,0 than the two closest (increase precision)
...
// public static double  DECONCV_SATURATION=     2.0;
 ...
 public static ImageStack convolutionKernelStack=null; // select to use for image convolution
  public Aberrations_all() {
    super("Aberrations_all");
...
/* Start with referenceComponent */
	  ...
	  if (DEBUG_LEVEL>2) {
	    System.out.println("3-PSF_shifts.length= "+PSF_shifts.length+" i="+i+" input_bayer.length="+input_bayer.length);
...
/* Now process all the other components */
	  ...
	  if (DEBUG_LEVEL>2) {
	    System.out.println("4-PSF_shifts.length= "+PSF_shifts.length+" i="+i+" input_bayer.length="+input_bayer.length);
		System.out.println("Before: color Component "+i+" PSF_shifts["+i+"][0]="+IJ.d2s(PSF_shifts[i][0],3)+
				  " PSF_shifts["+i+"][1]="+IJ.d2s(PSF_shifts[i][1],3));
	  }  
        if (DEBUG_LEVEL>2) {
...
//    double [][] test02;
      for (nTileY=0;nTileY<PSFBooleanMap.length;nTileY++) for (nTileX=0;nTileX<PSFBooleanMap[0].length;nTileX++) if (PSFBooleanMap[nTileY][nTileX]) {
        if (MASTER_DEBUG_LEVEL>0) IJ.showStatus("Processing tile["+nTileY+"]["+nTileX+"] ("+ncell+" of "+numPatternCells+")");
...
//TODO - use now       DoubleGaussianBlur
...
// get to frequency domain
  	 ...
  	 if ((DEBUG_LEVEL>3) && (title!="")) { /* Increase debug evel later */ // was 3
...
// phase(f2)=         -2*pi      *(Wx    * f2x            + Wy    * f2y           )
  ...
  public boolean showGaussianStackDialog() {
    ...
    gd.addNumericField("Inverted kernel size:",                                            INVERSE_REVERSE_SIZE, 0); //64
